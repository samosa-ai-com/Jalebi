...
- Server binds to **0.0.0.0** by default for local network (LAN) access (configurable via `JALEBI_HOST=127.0.0.1` or any custom interface IP). This is intentional — the owner uses the app from multiple machines (LAN, Tailscale, or a tunnel URL — all served by the same port).
- **Notification links follow `JALEBI_PUBLIC_URLS`** (optional comma-separated `Label=url` list, first = primary tap target; unset → `127.0.0.1` links). With several URLs configured, each push carries one `Open (<label>)` button per URL (max 3) so the tap works from any network.
