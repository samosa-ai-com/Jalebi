...
4. Drives the opencode adapter with the screen's `system_prompt` + a structured contract: *"return your findings as a single JSON array"*. Screens can pin a **backend** (`cli`, default `opencode`) and a **model** (optional); the engine resolves `get_adapter(screen.cli or "opencode")` and passes `model=screen.model or None`. Before building the prompt, the engine assembles the **known-open context** (`get_open_findings`): still-open findings from the last 3 done runs of every screen on the same repo (running screen first, dealt fingerprints excluded, cross-run duplicates dropped, 20 items / 400-char excerpts) rendered as a fenced UNTRUSTED block instructing the agent not to re-report the same issue — so a rerun after a fix commit audits blind only when there is nothing known open.
...
**Dealt state (server-side):** findings are marked dealt via `POST /api/screenings/dealt` (`{screen_id, fps}`, idempotent; `POST /dealt/reopen` reopens, `POST /dealt/import` is the one-time browser migration, `GET /dealt?screen_id=` lists) and stored in `screening_dealt` (see `docs/02`). Dealt drives three behaviors: inbox hide/show, the rerun context above, and notification filtering. The legacy per-browser localStorage set is import-once migrated on first Screenings visit, then the server is authoritative.
