...
- The queue builds the env via `_build_agent_env(token)`: for `issue_fix`/`pr_review` it carries the owner-PAT git creds (`GIT_CONFIG_*`) + `JALEBI_GITHUB_TOKEN`; **freeform/screen_finding runs are token-free** (`_agent_token_for` → `None`), so no `JALEBI_GITHUB_TOKEN` and no git `http.extraHeader` — the agent commits locally and Jalebi pushes/publishes. `GIT_AUTHOR_*`/`GIT_COMMITTER_*` = `Jalebi <jalebi@localhost>` always, plus the `gh`-neutralization vars. Inherited `GIT_CONFIG_*`/`GIT_DIR` state is stripped and `GIT_CONFIG_NOSYSTEM=1`/`GIT_CONFIG_GLOBAL=/dev/null` are pinned, so the parent shell's git config cannot hijack or redirect the agent's git; an inherited `JALEBI_GITHUB_TOKEN` is also stripped from token-free runs.
