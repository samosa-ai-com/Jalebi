...
```
cd apps/server && uv run pytest            # 629 passed (as of the Phase-3 commits)
cd .. && npm test -w @jalebi/web           # 63 passed
...
| 2.2 | Setting `default_backend` to **claude** → the form's Backend select defaults to **claude** and the Model dropdown lists the curated claude aliases (`default`, `sonnet`, `opus`, `haiku`, `sonnet[1m]`, `opus[1m]`, `best`). | Settings → Default backend → claude; open the New Task form. | ☐   |
...
| 4.3 | The guard files never appear in the agent's`git status`/commits (info/exclude + pre-commit hook, which also rejects forced stages of `.claude/settings.json` carrying the Jalebi hook and of `.claude/skills/`/`.codex/skills/`/`.agents/skills/`); a `freeform` codex task's final diff contains **no** `.codex/`, `opencode.json`, or `CLAUDE.md` entries. | `git -C <wt> status --porcelain` after a run; inspect the task's diff viewer. | ☐ |
| 4.4 | A**claude** task worktree contains **`.claude/settings.json`** with `permissions.deny` `Bash(gh*)` variants + sensitive-home-path `Read`/`Edit` rules, **`.claude/hooks/jalebi_deny_external.py`** (the `PreToolUse` external-dir hook), and **`CLAUDE.md`** with the Jalebi rules block, plus `AGENTS.md`. (Claude reads `CLAUDE.md`, not `AGENTS.md`.) | Bootstrap is testable via the unit suite (`test_write_claude_guard_writes_hook_and_deny_rules`, `test_claude_hook_denies_outside_worktree_and_allows_inside`); for a live check, run any claude task (will fail on auth — see §8) and inspect the worktree files before cleanup. | ☐ |
| 4.5 | **repo-owned files are never clobbered:** all three guards skip a repo-owned file of the same path — `.claude/settings.json`, `opencode.json`, `.codex/rules/default.rules` — and `remove_guard` (task delete) deletes a guard only while its content still matches exactly what Jalebi wrote, so repo-owned/edited files survive. | Covered by the automated suite (`test_*_does_not_clobber_repo_*`, `test_remove_guard_preserves_repo_owned_*`); manual spot-check optional. | ☐ |
| 4.6 | Deleting a task removes its guards (`.codex/`, `.claude/settings.json` only when Jalebi wrote it, the claude hook only when Jalebi wrote it, marker blocks stripped from `AGENTS.md`/`CLAUDE.md`, all skill roots) and prunes the worktree. | Delete a codex task; confirm`~/.jalebi/ws/task-<id>` is gone. | ☐ |
...
- **Claude live runs** cannot be validated here (no subscription available). The claude adapter is code-complete + unit-tested against captured fixtures; a real-auth smoke is the **only remaining manual gate** before merge (this includes the `PreToolUse` external-dir hook firing in a real `-p` run — its logic is unit-tested offline, but a live claude run has not observed it).
- **codex sandbox mode on this machine is `danger-full-access`** — user namespaces are blocked (`bwrap` fails), so `workspace-write` cannot write here. On a userns-capable machine codex runs under `sandbox_mode=workspace-write` (network enabled via config override). This is an automatic per-machine probe, not a setting; the fallback logs a one-time warning with the host-level fix (`sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0` or setuid bwrap).
- **codex model list** comes from the per-account `~/.codex/models_cache.json` (API-callable models preferred); arbitrary `-m` names (e.g. `gpt-4.1-mini`, `gpt-5-codex`) are rejected server-side for ChatGPT accounts — only the account's codex models work.
- **Claude disk confinement** is a **tool gate, not an OS boundary**: the `PreToolUse` hook + deny rules block the file tools (`Read`/`Write`/`Edit`/`Glob`/`Grep`) and sensitive home paths outside the worktree, but arbitrary **Bash subprocess file I/O** (e.g. `python3 -c "open(...)"`, `npm` touching `~/.npm`) is not confined — same limitation as opencode's `external_directory: deny`. OS-level sandboxing (`additionalDirectories`/bwrap) is trust-gated and intentionally not used.
