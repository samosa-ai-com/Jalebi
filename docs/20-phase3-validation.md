...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- | ---- |
| 1.1 | **Settings → Agent backend** is now a dropdown with **opencode / codex / claude** (was opencode-only).                                                                                                            | Settings page → "Agent backend".     | ☐Y  |
| 1.2 | The backend dropdown is one line: picking it switches the agent for new tasks (the**Task**, **Task detail follow-up**, **Screenings**, and **Agents** model dropdowns now reflect the active backend). | Switch backends and open a task form. | ☐Y  |
...
| 1.5 | The **New task** form caption shows the **active backend** (e.g. `codex · runs in a local worktree`), not a hardcoded "opencode".                                                                                              | With `agent.cli=codex`, open the New task form. | ☐   |
...
| 2.5 | **Per-form backend:** `GET /api/models?cli=codex` returns the codex list even when the global setting is opencode — the Screenings/Agents **Model dropdown follows the Backend you select in that form** (blank backend = global setting). Unknown `?cli=foo` → empty list, no 500. | In a Screen form, switch Backend to codex → the Model dropdown lists codex models. | ☐    |
| 2.6 | **`adapter_model_lists` override:** setting it (e.g. `{"codex": ["my-model"]}` via `POST /api/settings`) makes `/api/models` return the override for that cli **before** the adapter's own list — for both the global backend and `?cli=`. An explicit empty list clears the dropdown. | `curl -X POST /api/settings -d '{"key":"adapter_model_lists","value":{"codex":["gpt-override"]}}'` → `/api/models` and `/api/models?cli=codex`. | ☐    |
| 2.7 | **Unpinned screens follow the global setting:** a screen with **no** Backend pin runs on whatever `agent.cli` is set to (a pinned screen keeps its own backend). Changing the setting does **not** touch existing screens' pins. | Set `agent.cli=codex`; run a screen with blank Backend → it runs on codex; a screen pinned to opencode still runs on opencode. | ☐    |
