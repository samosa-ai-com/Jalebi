...
- **Empty-run guard:** exit-0 with stream activity but zero agent output (no message/tool_call/diff content — e.g. a banner-only goose run) fails the run with a stable `without producing any agent output` marker (also a non-retryable pattern, so recovery doesn't loop). A bare `done` with no stream at all keeps the old verdict.
