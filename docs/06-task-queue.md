...
- **`freeform`** — the user's text, plus the shared best-practices/hard-rules `AGENTS.md`. Runs **token-free** (no `JALEBI_GITHUB_TOKEN`, no git push credentials); commits locally, Jalebi pushes. `publish_mode` defaults to **`manual`**.
