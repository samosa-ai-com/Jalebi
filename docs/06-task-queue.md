...
- **Sender** (`src/jalebi/notify.py`): best-effort httpx POST to the configured ntfy endpoint; never raises into the queue. The **merged `ntfy_topic` setting** is either a bare topic (→ `https://ntfy.sh/<topic>`) or a full URL to a self-hosted server (`ntfy_url` was folded into it). Messages are masked (PATs + secret patterns) before send.
- **Terminal notifications:** when a run ends (`done` / `failed` / `timed_out` / `cancelled` / `needs_approval`), the final agent message + task summary (id, type, repo, PR link) are pushed, gated by `notify_on_done` / `notify_on_failed` / `notify_on_needs_approval`.
...
- **Test endpoint:** `POST /api/notify/test` sends a sample push and reports ok/error (Settings → Notifications → "Send test notification").
