...
5. Streams `handle.events()`; `step`/`message`/`tool_call`/`done`/`error` events are **masked at ingest** (PRD F17: **all PATs** + `secret_patterns`), broadcast live over the per-task SSE channel, and stored in `runs.steps_json` (bounded: 500 steps, 2000-char texts). Consecutive message chunks coalesce, but the buffer flushes before a combined entry could exceed 2000 chars, so no bounded chunk tail is lost. **`tool_call` is persisted too**, so a reload doesn't lose console lines. A `cancelled` kill's "exited with code -15" error is replaced with a clean "Run cancelled by user." message.
6. Terminal status from event stream **or** watchdog/cancel reason.
7. If `done` and **`publish_mode` says publish** (per-task `"auto"`/`"manual"`; `None` → the global `auto_publish` setting; `issue_fix` defaults to `"auto"`, freeform/manual types to `"manual"`): **only if the branch holds the task's OWN commits** — ahead of the **worktree base** (`source_branch` for freeform-family, `target_branch` for `issue_fix`; PR-head tasks keep the target comparison), never just ahead of the target — otherwise a task based on `development` targeting `main` would ship the whole source branch as its PR with zero agent commits (this happened once as PR #8) → `_publish(mode="new_pr")` (auto-publish always uses `new_pr`): recreate the worktree if it was cleaned, then **sync-before-push** — `merge_origin_into` fetches origin and merges `origin/<target>` into the task branch so the PR is up to date with target's progress and mergable. **On conflict** the merge is **aborted**, the conflicting files are surfaced ("PR would conflict with `<target>`: file1…"), the task goes to `needs_approval`, and nothing is pushed or opened — the user resolves via a follow-up asking the agent to merge `origin/<target>` and resolve, then publishes again. On a clean merge → `push_branch`, then **dedup** — if an **open** PR with `head=jalebi/<taskId>` already exists (same-repo, via `find_pr_by_head`) it is **reused** (never a second PR, even if an agent opened one), else `create_pr`. Closed/merged PRs reusing the branch name are **never** reused (a stale PR from a previous session/wipe must not swallow the publish — verified via `get_pr` defensively, and the same guard clears a stored-but-closed `task.pr_number`). Title/body come from the agent-written `.jalebi/pr.md` when present (`# <title>` + description) — **fallback** to the prompt. Body always gets `Closes #N` derived from **`tasks.issues_json`** (the linked issues, not a `#N` regex over the prompt — so a stray `#3` in the wording never produces a bogus `Closes`) + a Jalebi-task footer + `Co-authored-by`. For `issue_fix`, Jalebi **comments on each linked issue** linking the PR — **only when the PR is newly created**, never on a reuse/follow-up push, and never when the run ended asking the owner for approval (same approval gate as reviews: push + PR still happen, only the comment is skipped). On publish failure → task `needs_approval` (manual publish available).
   ...
   **Manual publish** dispatches to one of three modes (`apps/server/src/jalebi/queue.py:TaskQueue.publish_task`):
   - `new_pr` (default) — same flow as auto-publish.
    - `update_pr` — fast-forward (or merge) `jalebi/<id>` into the existing PR's head branch (`GitWorkspace.fast_forward_into`) and force-push with `--force-with-lease` (`GitWorkspace.push_existing_branch`). On conflict → `PublishConflict` (409). On lease failure → `PushLeaseFailed` (412, owner must re-fetch). No new PR opened; `Closes #N` not added; no issue comment. **Fork PRs:** when the head lives on a fork, the queue instead merges into the mirror-local `fork-pr-<N>` branch (`GitWorkspace.merge_task_into_fork_head`, recreated at the current PR head) and pushes to the fork URL with a head-SHA lease (`GitWorkspace.push_fork_head`); a fork with maintainer edits disabled is refused with a `PublishError` pointing at `new_pr` (see `docs/04` §8).
    ...
    **Workflow-scope pushes:** when any mode's push creates or updates `.github/workflows/*` but the account's PAT lacks the `workflow` scope (classic) / Workflows: read/write (fine-grained), GitHub refuses the push. The queue translates this into a `PublishError` with fix instructions (add the scope, refresh via GitHub page “update token”, republish) — the task goes to `needs_approval` and the timeline shows the guidance.
...
- **`freeform`** — the user's text, plus the shared best-practices/hard-rules `AGENTS.md`. Runs as the **selected account** (its PAT is in the agent env for GitHub API use; no git push creds). `publish_mode` defaults to **`manual`**. **Linked PR (optional):** if the creator links a PR, creation fetches the PR + its current **review comments** into `context_json` (masked, truncated to guard size) and   `AGENTS.md` embeds a `## Linked pull request` block (PR + body). The creation form's **"Address the review comments on this PR"** checkbox (checked by default on PR link; `address_reviews`, freeform-only) is authoritative: checked → a `### PR review comments to address` instruction with the comments embedded (or a self-fetch fallback when none were fetched at creation); unchecked → the PR stays background context (description only, no instruction). The **"Address reviewers"** button on a fixer task opens the New-task form prefilled (repo, freeform, PR, prompt, flag) instead of posting a follow-up. **Fork-PR base:** when the linked PR's head is not on origin (fork), the form offers `pr/<N>/head` as the source (`tasks.pr_head_source_number`); the worktree is based on the current PR head commit (`GitWorkspace.create_worktree_from_pr_head`), the target auto-resolves to the PR base, and publish defaults to pushing back to that PR (`update_pr`, with a `new_pr` fallback when the fork disallows maintainer edits). `issue_fix`/`pr_review` reject the sentinel (400). Tasks created before this existed (or whose context fetch failed) still get the linked PR number + a fetch-it-yourself note in `AGENTS.md`. The **effective prompt** also carries a small linked-PR pointer (PR number + `curl …/pulls/N/reviews` with `$JALEBI_GITHUB_TOKEN`), so a run still knows which PR even if an agent restores `AGENTS.md` away — hard-rule #10 forbids a working-tree restore of `AGENTS.md` for exactly this reason.
...
- **`pr_review`** — `TaskQueue._run_review`: the agent runs in a **detached review worktree** checked out at the PR head (`refs/pull/<n>/head`, works for fork PRs too), told to review only (never push). On success Jalebi reads `.jalebi/review.md` (fallback: last message) and posts it as a GitHub PR review **COMMENT** (`POST .../pulls/N/reviews`, event `COMMENT` — never approve/merge). **Review depth / nitpick mode:** the global `review_nitpick_mode` setting (default `true`, Settings → Queue & timeouts) controls review depth in `prompts.build_agent_md`. When `true`, the brief asks for a thorough review including minute nits (typos, style, micro-suggestions); when `false`, it instructs a short, high-signal review focused on blockers and significant issues only. The effective mode is recorded on the task context (`context_json["review_nitpick_mode"]`) at dispatch time for reproducibility — a per-task value already present in the context wins over the global setting and is left untouched. **Approval gate:** when no `review.md` exists and the last message asks the owner for approval (plan-first agents, e.g. codex obeying a repo `AGENTS.md` — detected via `attention.is_waiting_message`), NOTHING is posted; the run stays `done`, the assignment stays `running`, and approval surfaces as `needs_you` in the UI + the ntfy `notify_on_needs_approval` push until the owner approves via follow-up. Review tasks never publish. Requires `pr_number` at creation. **Reviewer assignments (Phase 1, PRD F7):** a reviewer task created via `reviews.assign_reviewers` (or `reviewers: [...]` on the create payload) carries a `review_assignments` row; `_run_review` marks it `running` (with the run) at start, `posted` on successful review posting, and `failed` on a run error. Each reviewer = one task, so multiple reviewers run in parallel under concurrency.
...
- **Stream coalescing:** consecutive `message` events merge into one timeline step (flushed on a non-message event, every ~500 chars, or at stream end) — word-streaming backends (grok) no longer produce hundreds of one-word steps. The same merged entries go to SSE and `steps_json`, so the durable timeline and the run cache never diverge.
- **Empty-run guard:** exit-0 with stream activity but zero agent output (no message/tool_call/diff content) fails the run with a stable `without producing any agent output` marker (also a non-retryable pattern, so recovery doesn't loop). A bare `done` with no stream at all keeps the old verdict.
- **Drain on error:** a mid-stream `error` no longer stops the drain — later events still record, the child is reaped, and a non-zero exit still surfaces its stderr tail. Any error in the stream still fails the run.
- **Follow-up rows:** recorded with the user's verbatim body (agent `custom_instructions` join only the prompt) plus the requested `cli` override; a follow-up that dies by exception after its run row exists records its row too (failed attempts stay visible).
...
- **Recovery budget:** `retry_count` resets on manual rerun, so a task that hit the attempt cap gets fresh auto-recovery instead of giving up on its next failure. Quota/limit failures (`usage limit`, `rate limit`, `429`, …) are non-retryable by default; `seed_defaults` backfills missing `retry_policy` sub-keys on existing installs without touching owner edits.
- **Missing binary fails fast:** after cli resolution the worker verifies the backend's CLI is on `PATH` (`_require_cli`); a missing binary fails the run with a clear non-retryable message recorded on the timeline — no spawn crash, no recovery loop. Pins to backends the registry no longer knows fall back to the first enabled backend (`_enabled_cli`).
...
- **Master switch (`ntfy_enabled`, default true):** channel-level on/off independent of the endpoint — off silences pushes without losing the configured topic, on resumes them with no re-configuration. Gating order per send is master switch → `ntfy_topic` non-empty → per-event toggle (`notify_on_done` / `notify_on_failed` / `notify_on_progress` / `notify_on_needs_approval`) or, for screenings, per-screen `notify_ntfy`. It covers terminal + progress pushes, screening finding pushes, and failed-login alerts; in-app bell rows are always recorded server-side regardless. Toggled via Settings → Notifications (Ntfy notifications card, `POST /api/settings` with a bool).
...
- **Backend allow-list (`enabled_backends`, seeded from installed CLIs, full registry when none found):** every Backend picker offers only enabled backends (`GET /api/backends`); the default backend can't be disabled and at least one must stay on. Strictness differs by layer on purpose: creation/rerun/follow-up *refuse* an uninstalled backend (nothing created), while dispatch *falls back* to the first enabled one (a backend disabled after pinning must still run instead of 500ing). A task/screen pinned to a backend disabled later falls back to the first enabled one at dispatch (`TaskQueue._enabled_cli`, logged) instead of failing.
