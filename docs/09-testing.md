...
- **Server (`apps/server/tests/`, pytest):** `test_health`, `test_config`, `test_schema` (migrations match models via `compare_metadata`), `test_settings`, `test_api_settings`, `test_secrets` (strict no-fallback resolution), `test_github` (client), `test_api_github` (equal-account model), `test_repos`, `test_api_repos` (connect requires an account), `test_git_workspace` (local git repos, incl. real `refs/pull/N` review worktree, stale-branch reset, and prune of missing-but-registered worktrees), `test_opencode_adapter` (incl. no server-env leak in `_spawn`), `test_runhandle`, `test_masking`, `test_prompts`, `test_worktree_bootstrap` (incl. `external_directory: deny` guard), `test_artifacts`, `test_queue` (incl. selected-account agent env, dirty-tree surfacing, per-task publish mode), `test_followups`, `test_reliability`, `test_api_tasks` (incl. publish-mode defaults/override, oversized-prompt rejection, delete, account required), `test_sse`, `test_spa`. Fixtures in `conftest.py`: `config` (tmp data dir), `app` (via `create_app(config)`), `client` (Flask test client), `session`, `engine`. (~263 tests; `uv run pytest`.)
