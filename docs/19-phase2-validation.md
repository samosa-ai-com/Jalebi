...
- [X] A GitHub PAT is bound to the test repo (GitHub page → account green). Recommended test repo: **`example-owner/example-repo`** (the designated test repo for QA). Screening fails with a clear error if no account is bound.
...
| 2.2 | Each screen card shows: name, the repo (`owner/repo` + `· branch` or `· default`), the cadence cron (mono), two status dots (**both green when on**: enabled + ntfy-on — the amber ntfy dot was confusing), and **Run now / History / Edit / Delete** buttons. | Create a screen (see 3) and look at the card. | ☐Y<br />It is present, but the status bars for NTFY show yellow even when it is on. |
...
| #   | What to expect                                                                                                                                                                                                                                                                                                                                         | How to test                                                                                                                                  | Pass                                                                                                                                                                                                                                                                                                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.1 | **New screen** opens the form with a **Starter template** dropdown listing all 7 built-in templates (Security posture, Dependency hygiene, Dead code & cruft, Test coverage gaps, Docs drift, Performance hotspots, Code-quality consistency). Picking one pre-fills name, cron, and system prompt **and the dropdown shows the chosen template** (it stays visible, and you can pick another anytime). | `+ New screen` → pick "Security posture" → name/cron/prompt fill in and the select shows "Security posture". | ☐When I select any element of the list from the starter template, it does not show that it is selected. However, it changes all other things in the form. So it is working, but just the selected starter template list name is not showing. You can also pick a starter screen even after picking one. |
| 3.2 | Form fields: Name, Repo (connected repos), **Scope branch** (a **dropdown** of the repo's branches from `GET /api/repos/<id>/branches`; blank = default), **Cadence (5-field cron)** with **preset chips** (`Every 5 min`, `Hourly`, `Daily 6 AM`, `Weekly Mon 6 AM`, `Weekdays 9 AM`) that fill the editable field, System prompt, **Backend** and **Model** (dropdown from `/api/models`, blank = default), and **Enabled** / **Notify on findings (ntfy)** checkboxes. | Create a screen against the test repo: pick a scope branch from the list, tap a cron preset. | ☐The scope branch should be a list of branches to select from, not just a free text field.<br />The cron entry is not something not everyone knows the syntax of it, so it's better if you actually provide some option to select from to create the cron pattern. |
...
| 3.4 | **Edit** — each card now has an **Edit** button that reopens the form with the **Repo locked**, so the **Enabled** and **Notify on findings (ntfy)** toggles (plus name/prompt/cron/branch/model) are reachable and save via `PUT /api/screenings/<id>`. | Click **Edit** on a screen → the form opens prefilled → toggle Enabled/Notify → **Save changes** → card updates. | ☐Y<br />At least add an option to enable and disable the notification and the screening. Otherwise, it is useless. |
...
| 4.3 | A run with**no findings** shows "No findings." **only once it is `done`** — an in-flight (`queued`/`running`) run shows a **"Running…"** indicator instead. The History section **auto-refreshes every 5s**, so a run's status flips to `done` without a manual page refresh. | Start a screen run → open History → see "Running…" while it runs, then it updates to the result on its own. | ☐Y<br />It shows no findings even when it is running. But I would expect it to only show the findings or no findings after it completes the task.<br />Also, the history status does not change from ready to done automatically. I need to manually refresh the page. |
...
> The scheduler is a daemon thread that wakes **every 60s** and runs enabled screens whose cron matches the current clock in the **app's timezone** — the `timezone` setting (Settings), defaulting to the machine's **local** zone. So a cron of `30 1 * * *` fires at 1:30 **local** (the earlier build matched UTC — a non-UTC owner's cadence never fired; now fixed). **Baseline dedup:** a screen skips a tick when its last terminal run (`done`/`failed`) audited the **same HEAD**. **"Run now" always forces** a run past the dedup watermark.
...
sha=$(curl -s -H "Authorization: Bearer $JALEBI_GITHUB_TOKEN" \
  "https://api.github.com/repos/example-owner/example-repo/pulls/<PR#>")
curl -s -H "Authorization: Bearer $JALEBI_GITHUB_TOKEN" \
  "https://api.github.com/repos/example-owner/example-repo/commits/<head_sha>/status"
...
- **Screening run events aren't streamed into the UI** — the SSE endpoint exists (`/api/screenings/runs/<id>/events`), but the Screenings page uses 5s polling. Re-open History to see a finished run if you close it mid-run.
...
- **Scheduler granularity** — the daemon wakes every 60s; a cron due mid-minute fires on the next tick. Cadence follows the **app `timezone` setting** (default: this machine's local zone).
...
# Phase 2 remediation (2026-08-15) — fixes from the first QA pass

Every comment you left in this checklist was addressed. The rows above already describe the new behavior; this is the quick reference:

| Your comment | Fix |
|---|---|
| **3.1** — "the selected starter template list name is not showing" | The template `<select>` was hardcoded to `value=""`. It is now controlled — the chosen template stays visible (and you can still pick another). |
| **3.2** — "scope branch should be a list of branches" | Scope branch is now a **dropdown** from `GET /api/repos/<id>/branches` (blank = repo default). |
| **3.2/3.3** — "cron … provide some option to select from" | Added **cron presets** (`Every 5 min`, `Hourly`, `Daily 6 AM`, `Weekly Mon 6 AM`, `Weekdays 9 AM`) + the editable field with a hint. |
| **3.4** — "add an option to enable/disable the notification and the screening, otherwise it is useless" | Each card now has an **Edit** button opening the form (repo locked) with the **Enabled** + **Notify on findings** toggles. |
| **§3 note** — "add an option to select the model and backend for this" | Screens now accept **Backend** (`cli`, default opencode) + **Model** pins (`/api/models`); new migration `a4b6c8d0e2f4`; the engine resolves `get_adapter(cli or "opencode")` and passes `model` to the run. |
| **§3 note** — "the schedule did not work … 1:30 AM … time zone … use the local time zone" | **Timezone root cause fixed.** The scheduler matched cron against **UTC**; your `30 1 * * *` never fired. All of Jalebi now runs on the **app timezone** (new `clock.py`), defaulting to the machine's **local** zone, with a **Timezone** setting (Settings) for an IANA override. Serialized timestamps carry the zone's offset so the browser renders them correctly. A screen scheduled for the next local minute now fires on the next scheduler tick. |
| **2.2** — "status bars for NTFY show yellow even when it is on" | The ntfy-on dot is now **green** when on (both "on" dots are green). |
| **4.3** — "shows no findings even when it is running" | In-flight (`queued`/`running`) runs show a **"Running…"** indicator; "No findings." only appears for a finished `done` run. |
| **4.3** — "history status does not change … need to manually refresh" | Run history **polls every 5s** while the section is open — the status flips to `done` automatically. |
