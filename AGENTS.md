...
- **Phase 4 — Visualization & control:** see **`docs/21-phase4-plan.md`**. Agent-"waiting for input" UX + markdown rendering (T0), backend hardening (T1), GitHub polling observer + derived attention (T2), frontend features (T3), deps/nudge/durable-SSE/live running view (T4), presentation fixes (T5), IDE connector (T6), in-task file browser (T7). **Status: complete on `phase-4` (8 commits, all pushed); not yet merged to `main`.**
